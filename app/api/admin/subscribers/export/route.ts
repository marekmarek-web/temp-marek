...
export async function GET() {
  ...
  const { data: rows, error } = await session.supabase
    .from("subscribers")
    .select(
      "id,created_at,updated_at,email,name,source,interest_segment,status,consent_marketing_at,source_path,related_lead_id",
    )
  ...
  for (const row of rows ?? []) {
    ...
    lines.push(headers.map((h) => csvEscape(r[h] == null ? "" : String(r[h]))).join(","));
  }
